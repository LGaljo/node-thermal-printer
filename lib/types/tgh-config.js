module.exports = {
  ...
  CTL_LF: Buffer.from([0x0a]), // Print and line feed OK
  CTL_FF: Buffer.from([0x0c]), // Form feed OK
  CTL_CR: Buffer.from([0x0d]), // Print and carriage return OK
  CTL_HT: Buffer.from([0x09]), // Horizontal tab OK
  CTL_SET_HT: Buffer.from([0x1b, 0x44]), // Set horizontal tab positions
  CTL_VT: Buffer.from([0x1b, 0x64, 0x04]), // Print and feed paper n rows 0 <= n <= 255
  CTL_VTf: (n) => Buffer.from([0x1b, 0x64, n]), // Print and feed paper n rows 0 <= n <= 255

  // Real Time Status transmission
  RT_ST_TX_PRINTER: Buffer.from([0x10, 0x04, 0x01]), // Transmit printer status
  RT_ST_TX_OFFLINE: Buffer.from([0x10, 0x04, 0x02]), // Transmit off-line status
  RT_ST_TX_ERROR: Buffer.from([0x10, 0x04, 0x03]), // Transmit error status
  RT_ST_TX_PAPER_ROLL: Buffer.from([0x10, 0x04, 0x04]), // Transmit paper roll sensor status
  RT_ST_TX_FULL_STATUS: Buffer.from([0x10, 0x04, 0x14]), // Transmit FULL STATUS
  RT_ST_TX_PRINT: Buffer.from([0x10, 0x04, 0x11]), // Transmit print status
  RT_ST_TX_PRINTER_ID: Buffer.from([0x10, 0x04, 0x15]), // Transmit printer ID
  RT_ST_TX_EJECTOR_MOTOR: Buffer.from([0x10, 0x04, 0x0b]), // Transmit ejector motor status

  // Printer hardware
  HW_INIT: Buffer.from([0x1b, 0x40]), // Clear data in buffer and reset modes
  HW_SELECT: Buffer.from([0x1b, 0x3d, 0x01]), // Printer select
  HW_RESET: Buffer.from([0x1b, 0x3f, 0x20]), // Reset printer hardware
  ...
  LED_BAR_ON: Buffer.from([0x1b, 0x48, 0x01]), // Led bar ON (if equipped)
  LED_BAR_OFF: Buffer.from([0x1b, 0x48, 0x00]), // Led bar OFF (if equipped)
  ...
  PPC_AREA_WIDTH: (nL, nH) => Buffer.from([0x1d, 0x57, nL, nH]), // Set printing area width

  // MECHANISM CONTROL COMMANDS
  PAPER_FULL_CUT: Buffer.from([0x1b, 0x69]), // Total cut
  PAPER_PART_CUT: Buffer.from([0x1b, 0x6D, 0x01]), // Total cut
  PAPER_FULL_CUT_BACK: Buffer.from([0x1c, 0xc0, 0x34]), // Total cut and automatic paper moving back

  // POWER MANAGEMENT
  LOW_POWER_ON: Buffer.from([0x1c, 0xef, 0xae, 0x01]),
  LOW_POWER_OFF: Buffer.from([0x1c, 0xc0, 0x34]),

  // Text format
  TXT_NORMAL: Buffer.from([0x1b, 0x21, 0x00]), // Normal text - character font A
  TXT_NORMAL_B: Buffer.from([0x1b, 0x21, 0x01]), // Normal text - character font B
  TXT_EXPANDED: Buffer.from([0x1b, 0x21, 0x08]), // Normal text - character font B
  TXT_2HEIGHT: Buffer.from([0x1b, 0x21, 0x10]), // Double height text OK
  TXT_2WIDTH: Buffer.from([0x1b, 0x21, 0x20]), // Double width text OK
  TXT_SPACING: (n) => Buffer.from([0x1b, 0x20, n]), // Set right-side character spacing [0,255]

  TXT_ITALIC_ON: Buffer.from([0x1b, 0x34, 0x01]), // Italic text ON
  TXT_ITALIC_OFF: Buffer.from([0x1b, 0x34, 0x00]), // Italic text OFF
  ...
  TXT_DBL_STRIKE_ON: Buffer.from([0x1b, 0x47, 0x01]), // Double strike ON
  TXT_DBL_STRIKE_OFF: Buffer.from([0x1b, 0x47, 0x00]), // Double strike OFF
  ...
  CODE_PAGE_PC437_USA:              Buffer.from([0x1b, 0x74, 0x00]),
  CODE_PAGE_KATAKANA:               Buffer.from([0x1b, 0x74, 0x01]),
  CODE_PAGE_PC850_MULTILINGUAL:     Buffer.from([0x1b, 0x74, 0x02]),
  CODE_PAGE_PC860_PORTUGUESE:       Buffer.from([0x1b, 0x74, 0x03]),
  CODE_PAGE_PC863_CANADIAN_FRENCH:  Buffer.from([0x1b, 0x74, 0x04]),
  CODE_PAGE_PC865_NORDIC:           Buffer.from([0x1b, 0x74, 0x05]),
  CODE_PAGE_VISCII:                 Buffer.from([0x1b, 0x74, 0x06]),
  ...
  CODE_PAGE_SPACE_PAGE:             Buffer.from([0x1b, 0x74, 0xFF]),
  CODE_PAGE_INTL_USA:               Buffer.from([0x1b, 0x52, 0x00]),
  CODE_PAGE_INTL_FRANCE:            Buffer.from([0x1b, 0x52, 0x01]),
  CODE_PAGE_INTL_GERMANY:           Buffer.from([0x1b, 0x52, 0x02]),
  CODE_PAGE_INTL_UK:                Buffer.from([0x1b, 0x52, 0x03]),
  CODE_PAGE_INTL_DENMARK1:          Buffer.from([0x1b, 0x52, 0x04]),
  CODE_PAGE_INTL_SWEDEN:            Buffer.from([0x1b, 0x52, 0x05]),
  CODE_PAGE_INTL_ITALY:             Buffer.from([0x1b, 0x52, 0x06]),
  CODE_PAGE_INTL_SPAIN1:            Buffer.from([0x1b, 0x52, 0x07]),
  CODE_PAGE_INTL_JAPAN:             Buffer.from([0x1b, 0x52, 0x08]),
  CODE_PAGE_INTL_NORWAY:            Buffer.from([0x1b, 0x52, 0x09]),
  CODE_PAGE_INTL_DENMARK2:          Buffer.from([0x1b, 0x52, 0x0A]),
  ...
  // Graphic bank
  // m: 0 <= m <= 3: logo number
  // n: 0 <= n <= 3: print mode (0 - normal, 1 - double width, 2 - double height, 3 - double width and double height)
  LOGO: (m, n) => Buffer.from([0x1b, 0xfa, m, n]),
  // LOGO: this.LOGO_f(0, 0), // 1. logo in mormal mode
  ...
  // BARCODE_HEIGHT: this.BARCODE_HEIGHT_f(0x64), // Barcode Height [100]
  // BARCODE_HEIGHT_f: (n) => Buffer.from([0x1d, 0x68, n]), // Barcode Height [1-255]
  // BARCODE_WIDTH: this.BARCODE_WIDTH_f(0x03), // Barcode Width  [2-6]
  // BARCODE_WIDTH_f: (n) => Buffer.from([0x1d, 0x77, n]), // Barcode Width  [2-6], data follows d1...df
  BARCODE_HEIGHT: Buffer.from([0x1d, 0x68, 0x64]), // Barcode Height [100]
  BARCODE_WIDTH: Buffer.from([0x1d, 0x77, 0x03]), // Barcode Width  [2-6]
  BARCODE_UPC_A: Buffer.from([0x1d, 0x6b, 0x00, 0x00]), // Barcode type UPC-A, data follows d1...df
  BARCODE_UPC_E: Buffer.from([0x1d, 0x6b, 0x01, 0x00]), // Barcode type UPC-E, data follows d1...df
  BARCODE_EAN13: Buffer.from([0x1d, 0x6b, 0x02, 0x00]), // Barcode type EAN13, data follows d1...df
  BARCODE_EAN8: Buffer.from([0x1d, 0x6b, 0x03, 0x00]), // Barcode type EAN8, data follows d1...df
  BARCODE_CODE39: Buffer.from([0x1d, 0x6b, 0x04, 0x00]), // Barcode type CODE39, data follows d1...df
  BARCODE_ITF: Buffer.from([0x1d, 0x6b, 0x05, 0x00]), // Barcode type ITF, data follows d1...df
  BARCODE_CODABAR: Buffer.from([0x1d, 0x6b, 0x06, 0x00]), // Barcode type CODABAR, data follows d1...df
  BARCODE_CODA93: Buffer.from([0x1d, 0x6b, 0x07, 0x00]), // Barcode type CODABAR, data follows d1...df
  BARCODE_CODA128: Buffer.from([0x1d, 0x6b, 0x08, 0x00]), // Barcode type CODABAR, data follows d1...df
  BARCODE_CODA32: Buffer.from([0x1d, 0x6b, 0x14, 0x00]), // Barcode type CODABAR, data follows d1...df
  ...
  QRCODE_CORRECTION_A: Buffer.from([0x1d, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x45, 0x30]), // Correction level: AUTO
  QRCODE_CORRECTION_L: Buffer.from([0x1d, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x45, 0x31]), // Correction level: L - 7%
  QRCODE_CORRECTION_M: Buffer.from([0x1d, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x45, 0x32]), // Correction level: M - 15%
  QRCODE_CORRECTION_Q: Buffer.from([0x1d, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x45, 0x33]), // Correction level: Q - 25%
  QRCODE_CORRECTION_H: Buffer.from([0x1d, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x45, 0x34]), // Correction level: H - 30%
  ...
  QRCODE_CELLSIZE_f: (n) =>  Buffer.from([0x1d, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x43, n]),
  ...
  PD_N37: Buffer.from([0x1d, 0x7c, 0x01]), // Printing Density -37%
  PD_N25: Buffer.from([0x1d, 0x7c, 0x02]), // Printing Density -25%
  PD_N12: Buffer.from([0x1d, 0x7c, 0x03]), // Printing Density -12%
  PD_0: Buffer.from([0x1d, 0x7c, 0x04]),   // Printing Density   0%
  PD_P50: Buffer.from([0x1d, 0x7c, 0x08]), // Printing Density +50%
  PD_P37: Buffer.from([0x1d, 0x7c, 0x07]), // Printing Density +37%
  PD_P25: Buffer.from([0x1d, 0x7c, 0x06]), // Printing Density +25%
  PD_P12: Buffer.from([0x1d, 0x7c, 0x05]), // Printing Density +12%
};
